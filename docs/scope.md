...
Scope 实例通过 `(scope_class_id, scope_instance_id)` 作为缓存 key：

```python
# cache_key 格式："scope_class_id:scope_instance_id"
# 例如：
cache_key = "global:global"  # GlobalScope
cache_key = "exchange:okx/main"  # ExchangeScope
cache_key = "trading_pair:okx/main:BTC/USDT"  # TradingPairScope
```

**重要**：相同的 `(scope_class_id, scope_instance_id)` 会复用同一个 Scope 实例，**即使 parent 不同**。这意味着：
- 同一个 Scope 可以被多个 parent 共享
- 这允许 Scope 在不同的链路中复用，提高效率
