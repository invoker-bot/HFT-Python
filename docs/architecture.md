...
| [executor.md](executor.md) | 执行器：DefaultExecutor、数据驱动设计 |
...
│   ├── config.py       # 策略配置
│   ├── static_positions.py  # 静态目标仓位策略
│   └── market_neutral_positions.py  # 市场中性策略
│
├── executor/       # 执行器模块
│   ├── base.py         # BaseExecutor 基类（数据驱动）
│   └── default_executor/   # 默认执行器
│
├── indicator/      # 指标模块（统一架构）
