...
- [ ] 修复：`BaseIndicator` 支持 `window: null`（None -> 0 归一化）（待实现）
- [ ] 测试：新增单元测试覆盖 `window=None` 与 `window=0` 的等价性（待实现）
- [ ] 回归：确认 `FairPriceIndicator` 等“单值 indicator”不再因 window 触发 TypeError（待审核）
