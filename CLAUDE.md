...
2. **审核阶段**：
   - 用户或审核者审核代码
   - **通过**：审核者将状态改为"（已通过）"并勾选 `[x]`
   - **驳回**：审核者将状态改为"（审核不通过）"并写明理由（此时审核未完成）；实现方继续修复后回到"（待审核）"
   - **争议**：实现方可将状态改为"（待商议）"并写明理由，等待老板介入裁决；老板裁决认可审核理由后，该条回到"（待审核）"继续复审流程
...
7. **代码审核**：提交前检查 `features/` 与 `issue/` 的待审核项；审核通过将状态改为"已通过"并勾选，若驳回需写明理由
...
| **已通过** | 用户/审核者审核通过 | `[x]` 勾选 | `- [x] 添加配置验证（已通过）` |
| **审核不通过** | 审核驳回（审核未完成，需要继续修复） | `[ ]` 未勾选 | `- [ ] 处理异常边界（审核不通过）` |
| **待商议** | 审核与实现对结论有争议，等待老板裁决 | `[ ]` 未勾选 | `- [ ] 争议点记录（待商议）` |
...
1. **实现角色**编写任务时：
   - 创建任务 → 标记为"（待实现）"
   - 完成实现 → 改为"（待审核）"，向用户报告
   - **禁止**自行改为"（已通过）"或勾选 checkbox

2. **审核角色**审核任务时：
   - 审核通过 → 改为"（已通过）"并勾选 `[x]`
   - 审核驳回 → 改为"（审核不通过）"并说明理由（此时审核未完成）
...
## 全局状态与 REVIEW.md 看板（避免漏审/重复审）

为避免“同一份 feature/issue 被反复全文复审”或“漏掉待复核项”，必须维护根目录 `REVIEW.md` 看板：

1. `REVIEW.md` 必须包含两张表：Features / Issues（各行记录：ID、文件路径、状态、备注）。
2. `REVIEW.md` 用于避免下次忘记“哪些号已通过/卡在哪”；为节省篇幅，允许把连续编号合并为区间（如 `0001-0010 已通过`）。仅当不同文件状态不同或需要备注时才拆分为多行。
3. 若某个“全部通过”的文件后续被修改：必须先移除该文件的 `> **状态**：全部通过`，并将受影响任务条目回退到 `（待审核）`/`（待实现）`；同时把该文件重新加入 `REVIEW.md` 对应表中。

### 文件级标记（避免重复阅读）

当某个 `features/*.md` 或 `issue/*.md` 文件中：
- 所有任务条目均为 `- [x] ...（已通过）`
- 且不存在任何 `（待实现/待审核/审核不通过/待商议）` 条目
...
表示该文件无需重复复审；若后续变更该文件则按前述规则回退状态并重新进入 `REVIEW.md` 看板。
