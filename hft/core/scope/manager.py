...
class ScopeManager(Listener):
    ...
    __pickle_exclude__ = {*Listener.__pickle_exclude__, "_cache", "_cache_state"}
    ...
    def get_class(self, class_name: str) -> Optional[type[BaseScope]]:
        """
        获取注册的 Scope 类
        ...
        Returns:
            Scope 类对象，未注册则返回 None
        """
        return BaseScope.classes.get(class_name, None)
    ...
    def get_or_create(
        ...
        **kwargs
        ...
        Args:
            class_name: Scope 类名（如 "GlobalScope"）
            class_id: Scope 类型 ID（用户在配置中定义，如 "global", "my_scope"）
            instance_id: Scope 实例 ID
        Returns:
            Scope 实例
        """
        # 使用 (scope_class_id, scope_instance_id) 作为缓存 key
        cache_key = BaseScope.calculate_id(class_name, instance_id)
        app_core = self.root
        # 从缓存中获取
        ...
        # 获取类对象
        scope_class = self.get_class(class_name)
        if scope_class is None:
            raise ValueError(f"Unknown scope class: {class_name}")
        kwargs.update(
            {
    ...
    def get(
        ...
        class_name: str,
        instance_id: ScopeInstanceId,
        default: Optional[BaseScope] = None
    ) -> Optional[BaseScope]:
        """
        获取 Scope 实例（不创建）
