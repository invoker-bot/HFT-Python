...
价格单位说明：
- 所有价格偏离使用相对值（比例），而非绝对价格
- 例如：price_diff = (trade_price - ref_price) / ref_price
- 这样 k 的单位是"每单位相对价格偏离"，与 AS 模型一致
"""
import math
import time
import numpy as np
...
class TradeIntensityCalculator:
    ...
    使用方法：
        calculator = TradeIntensityCalculator(config)
        calculator.update(trades_datasource, orderbook_datasource)
        if calculator.is_ready:
            result = calculator.result
            k = result.buy_k  # 用于 AS 公式
    """
