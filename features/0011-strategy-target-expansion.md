...
3. **去特殊化**：Strategy 输出为通用字典字段；StrategyGroup 聚合为列表（不对字段做特殊化）
4. **显式聚合**：Executor 可在 `vars/condition` 中显式聚合 `strategies[...]`（仍保留默认聚合用于向后兼容）
...
- Executor 无法自定义聚合方式
- 多个 Strategy 的输出如何合并？（sum? mean? max?）
...
# StrategyGroup 聚合多个 Strategy
# Strategy A: {"position_usd": 100, "speed": 0.1}
# Strategy B: {"position_usd": 200, "speed": 0.2}
# 聚合结果：
{("okx/main", "BTC/USDT"): {"position_usd": [100, 200], "speed": [0.1, 0.2]}}

# Executor 显式聚合
vars:
  - name: position_usd
    value: sum(strategies["position_usd"])  # [100, 200] → 300
  - name: speed
    value: avg(strategies["speed"])         # [0.1, 0.2] → 0.15
```
...
- 当前实现中，BaseExecutor 会先用默认聚合从 `strategies_data` 计算 `target_usd/speed/delta_usd` 并决定是否执行；`vars/conditional_vars/condition` 主要用于“条件判断 + 动态参数”，不会反向覆盖 BaseExecutor 的目标仓位计算逻辑。
...
# Strategy B 输出：
{("okx/main", "BTC/USDT"): {"position_usd": 200, "speed": 0.2, "urgency": 0.6}}

# StrategyGroup 聚合为：
{
    ("okx/main", "BTC/USDT"): {
        "position_usd": [100, 200],
        "speed": [0.1, 0.2],
        "urgency": [0.8, 0.6]
    }
}
...
3) 对每个 concrete `(exchange_path, symbol)`：
   - 收集该 exchange/symbol 上下文变量（requires/vars/conditional_vars/indicator 等）
   - 计算并输出字段 dict（如 `position_usd/speed/...`）
4) StrategyGroup：把多个 Strategy 的输出按 `(exchange_path, symbol)` 聚合成 `strategies["field"] = [v1, v2, ...]` 列表
...
- **生效规则**：
  ...
  - 若你需要“全局条件对每个 target 都用各自上下文求值”的行为，请把表达式下放到 target 级 `condition`（或拆为多个 Strategy）。
...
- [x] `StrategyGroup.get_aggregated_targets()` 已支持列表聚合（已通过）
