...
7. **代码审核**：提交前检查 `features/`、`issue/` 中的待审核项；审核通过将状态改为"已通过"并勾选，若驳回需写明理由
...
### 状态流转
- **待实现** → 实现角色完成后 → **待审核** → 审核角色通过后 → **已通过**（勾选 `[x]`）
...
- 当某个 `features/*.md` 或 `issue/*.md` 文件中：所有任务条目均为 `- [x] ...（已通过）`，且不存在任何 `（待实现/待审核/审核不通过/待商议）` 条目时：必须在文件开头加一行 `> **状态**：全部通过`，表示该文件无需重复复审。
- 若后续对该文件新增/修改任何内容（包括新增任务、变更验收口径、变更结论）：必须先移除 `> **状态**：全部通过`，并将受影响的任务条目回退到 `（待审核）`（或新增为 `（待实现）`），防止“已通过”状态掩盖新变更。

### REVIEW.md 看板（避免漏审/重复审）

- 必须维护根目录 `REVIEW.md`：包含两张表（Features / Issues），用于列出当前“需要再次处理/复核”的 `features/*.md` 与 `issue/*.md`。
- 表格内容必须省略“编号更靠前且已稳定”的文件：凡文件头含 `> **状态**：全部通过` 的，且编号更靠前、无需再读的，不应重复列出（示例：若 `0001-0003` 均已稳定，则从 `0004` 开始列）。
- `REVIEW.md` 的表格用于防止下次忘记“哪些号已通过/卡在哪”；为节省篇幅，允许把连续编号合并为区间（如 `0001-0010 已通过`）。仅当不同文件状态不同或需要备注时才拆分为多行。
- 若某文件后续被修改：必须先移除该文件的 `> **状态**：全部通过`，并把受影响的任务条目回退到 `（待审核）`/`（待实现）`；同时把该文件重新加入 `REVIEW.md` 对应表中。
